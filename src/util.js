...
export const withNoPadding = (component) => (<div className="sb-no-padding">{component}</div>); // eslint-disable-line
export const trapFocus = (element, prepend, append) => {
  if (!element) { return };
  const KEYCODE_TAB = 9;
  const focusableEls = Array.prototype.slice.call(
    element.querySelectorAll('a[href]:not([disabled]), button:not([disabled]), textarea:not([disabled]), input[type="text"]:not([disabled]), input[type="radio"]:not([disabled]), input[type="checkbox"]:not([disabled]), select:not([disabled])')
  );
  if (prepend) { focusableEls.unshift(document.querySelector(prepend)) }
  if (append) { focusableEls.push(document.querySelector(append)) };
...
export const scrollElement = (element, to, scrollType, duration) => {
  if (!element) { return };
  const easing = (t, b, c, d) => { return c * (t / d) * (t / d) + b };
  const startDate = new Date().getTime();
  const start = element[scrollType];
  const animateScroll = () => {
    const currentTime = new Date().getTime() - startDate;
    element[scrollType] = Number(easing(currentTime, start, to - start, duration)); // eslint-disable-line no-param-reassign
    if (canUseDOM() && currentTime < duration) { window.requestAnimationFrame(animateScroll); }
    else { element[scrollType] = to; } // eslint-disable-line no-param-reassign
  };
  animateScroll();
};
